<?php
/**
 * Class Frankly_AMP_Iframe_Sanitizer
 *
 * @package AMP
 */
 ...
 */
class Frankly_AMP_Iframe_Sanitizer extends AMP_Base_Sanitizer {
		...
		foreach ( $attributes as $name => $value ) {
			switch ( $name ) {
				case 'src':
					...
					break;
				case 'style':
					// Modified by Frankly development team 
					if( self::parse_style_attr( $value, 'height') !== null ){
						$out[ 'height' ] = self::parse_style_attr( $value, 'height' );
					}
					if( self::parse_style_attr( $value, 'width') !== null ){
						$out[ 'width' ] = self::parse_style_attr( $value, 'width' );
					}
					break;
				case 'width':
				case 'height':
	...
	/**
	 * Modified by Frankly development team.
	 * Enable height and width from style attribute.
	 * https://worldnow.atlassian.net/browse/SERVICES-872
	 *
	 * @param string  $string All properties specified in style attribute.
	 * @param string  $prop_name Property name (height or width).
	 * 
	 * @return integer|null Value from style attribute or null if not provided.
	 */
	private function parse_style_attr( $string, $prop_name ) {
		$style_props = explode( ';', $string );
		$output = null;
		if( is_array( $style_props )){
			foreach( $style_props as $prop ){
				if ( stripos( strtolower( $prop ), $prop_name ) !== false ) {
					$value = preg_replace( '/[^0-9]/', '', $prop );
					if( is_int( intval( $value ) ) ){
						if ( stripos( strtolower( $prop ), 'px' ) !== false ){
							$output = $value;
						}
					}
				}
